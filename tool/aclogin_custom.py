...
class AccTool(ToolBase):
    ...
    def default_cookie_path() -> pathlib.Path:
        ...
        config_dir = pathlib.Path(
            subprocess.check_output(["acc", "config-dir"], text=True).strip()
        )
    ...
    def store_session(self, cookie_value: str) -> bool:
        ...
        try:
            ...
            # 既存のJSONファイルがあれば読み込む
            session_data = {
                "cookies": ["REVEL_FLASH=", f"REVEL_SESSION={cookie_value}"]
            }
            ...
            if self.cookie_path.exists():
                try:
                    ...
                    for i, cookie in enumerate(session_data.get("cookies", [])):
                        ...
                        session_data.setdefault("cookies", []).append(
                            f"REVEL_SESSION={cookie_value}"
                        )

                        # REVEL_FLASHがなければ追加
                        if not any(
                            c.startswith("REVEL_FLASH=")
                            for c in session_data.get("cookies", [])
                        ):
                            session_data.setdefault("cookies", []).insert(
                                0, "REVEL_FLASH="
                            )
                except Exception:
                    # 読み込みに失敗した場合は新規作成
...
def main():
    parser = argparse.ArgumentParser(
        description="AtCoder の REVEL_SESSION クッキーを各種ツールに保存します"
    )
    parser.add_argument(
        "--tools",
        nargs="+",
        help="クッキーを保存するツール名（指定なしの場合は自動検出）",
    )

    # ツール固有のオプションを追加
    parser.add_argument(
        "--oj-cookie-path", type=str, help="oj のクッキーファイルのパス"
    )
    parser.add_argument(
        "--acc-cookie-path", type=str, help="acc のセッションファイルのパス"
    )
    ...
    if success_count == len(tools):
        print(
            f"✅ すべてのツール ({success_count}/{len(tools)}) にクッキーを保存しました"
        )
